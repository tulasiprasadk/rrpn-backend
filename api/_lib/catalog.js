...
export async function getCatalog() {
  ...
  const raw = decodeJsonBuffer(await fs.readFile(file));
  const parsed = JSON.parse(raw);
  const rows = Array.isArray(parsed?.value) ? parsed.value : Array.isArray(parsed) ? parsed : [];
...
export async function getProductById(id) {
  const rows = await getCatalog();
  return rows.find((product) => String(product.id) === String(id)) || null;
}
...
export async function getCategories() {
  ...
  for (const product of rows) {
    ...
    const id = category.id || product.CategoryId || product.categoryId || category.name || "general";
...
export function normalizeProduct(product) {
  const categoryName = product.Category?.name || product.category || product.categoryName || "General";
  const categoryId = product.Category?.id || product.CategoryId || product.categoryId || categoryName;
  return {
    ...product,
