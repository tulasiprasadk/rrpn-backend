...
const defaultSupplierCallback = backendUrl
  ? `${backendUrl}/api/suppliers/auth/google/callback`
  : '';
const defaultCustomerCallback = backendUrl
  ? `${backendUrl}/api/customers/auth/google/callback`
  : '';
const supplierCallbackURL = GOOGLE_CALLBACK_URL || process.env.GOOGLE_SUPPLIER_CALLBACK_URL || defaultSupplierCallback;
const customerCallbackURL = GOOGLE_CALLBACK_URL || process.env.GOOGLE_CUSTOMER_CALLBACK_URL || defaultCustomerCallback;
...
if (googleConfigured) {
  ...
  try {
    passport.use('supplier-google', new GoogleStrategy({
      ...
      callbackURL: supplierCallbackURL,
  ...
  console.warn('Google OAuth not configured: set GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, BACKEND_URL, and callback env vars to enable.');
}
